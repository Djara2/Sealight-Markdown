...
int main(void)
{
	...
	//                bold_initiated variable. Then, skip ahead 2 characters so that we skip the first and
	//                second asterisk.
	while(i < source_len - 1)
	{
		...
		if(source[i] == '*' && source[i+1] == '*')
		{
			...
			if(bold_initiated) concatenate(&html, &html_len, &html_capacity, "<b>");
			else concatenate(&html, &html_len, &html_capacity, "</b>");
