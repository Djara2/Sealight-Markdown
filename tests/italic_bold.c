...
int main(int argc, char *argv[])
{
	...
	//                italic_bold_initiated variable. Then, skip ahead 2 characters so that we skip the first and
	//                second asterisk.
	while(i < source_len - 2)
	{
		...
		if(source[i] == '*' && source[i+1] == '*' && source[i+2] == '*')
		{
			...
			if(italic_bold_initiated) concatenate(&html, &html_len, &html_capacity, "<b><i>");
			else concatenate(&html, &html_len, &html_capacity, "</i></b>");
