...
// 2. modified to add a SECOND NULL after the first. This prevents there being issues 
//    when trying to peek ahead with a deflist
char** split_string(char *source, char *delimiter)
{
	...
	// hotfix: add second NULL so that there are no crashes with trying to peek ahead 
	len++;
	if(len >= capacity)
	{
		capacity++;
